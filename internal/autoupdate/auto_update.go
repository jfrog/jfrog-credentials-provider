...
func AutoUpdate(logs *logger.Logger, client *http.Client, ctx context.Context, Version string) {
	...
	jfrogPluginReleasesUrl := utils.GetEnvs(logs, "JFROG_CREDENTIAL_PROVIDER_RELEASES_URL", "https://releases.jfrog.io/artifactory/jfrog-credential-provider")
	...
	err = downloadLatestBinary(ctx, logs, client, latestBinaryVersionAvailable, newBinaryPath, newBinarySigPath, jfrogPluginReleasesUrl, downloadSuffix)
	if err != nil {
