...
func main() {
	...
	// Create a subcommand for backup-config
	backupConfigCmd := flag.NewFlagSet("backup-config", flag.ExitOnError)
	backupIsYaml := backupConfigCmd.Bool("yaml", false, "Config is in YAML format")
	backupProviderHome := backupConfigCmd.String("provider-home", "", "Provider home directory")
	backupProviderConfig := backupConfigCmd.String("provider-config", "", "Provider config file name")
	...
	case len(os.Args) > 1 && os.Args[1] == "backup-config":
		backupConfigCmd.Parse(os.Args[2:])
		resolvedHome, resolvedConfig := provider.ProcessProviderConfigEnvs(*backupProviderHome, *backupProviderConfig)
		logs, err := logger.NewLogger()
		if err != nil {
			log.Fatalf("Failed to initialize logger: %v", err)
		}
		if err := provider.BackupConfig(*backupIsYaml, resolvedHome, resolvedConfig, logs); err != nil {
			logs.Exit("Failed to backup config: "+err.Error(), 1)
		}
		return
