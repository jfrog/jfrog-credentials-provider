provider "kubernetes" {
    ## Change this to take config file from the local machine
    ## Assumes K8s admin and not AWS admin - grant least privilege
    ## or give an option to use the AWS admin - but more options means more complexity
    config_path = var.kubeconfig_path
    # config_path = "~/.kube/config" # Default path for kubeconfig
}
